...
exports.getSalesReport = async (req, res) => {
  try {
    ...
    // A Sale record is counted only when a booking is Completed,
...
exports.debugSalesStatus = async (req, res) => {
  try {
    ...
    const salesStatus = {
      withValidBookings: allSales.filter(s => s.booking && s.booking.status === 'Completed').length,
    ...
    const validMonthSales = monthSales.filter(s =>
      s.booking && s.booking.status === 'Completed'
    );
