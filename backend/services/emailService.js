...
// ── Brevo HTTP helper ────────────────────────────────────
// sendContactEmail uses this instead of nodemailer/SMTP because
// Render blocks outbound port 587. Verification emails are unaffected.
const brevoSend = async ({ to, replyTo, subject, html, senderName = "Catherine's Oasis" }) => {
  const payload = {
...
// Uses Brevo HTTP API — Render blocks SMTP port 587 but HTTPS is fine.
const sendContactEmail = async ({ name, email, phone, subject, message }) => {
  ...
  // Send both via Brevo HTTP API in parallel.
  // Auto-reply failure is non-fatal; admin email failure throws so the route returns 500.
  const results = await Promise.allSettled([
    brevoSend({
      to:         ADMIN_EMAIL,
      replyTo:    email,
      senderName: "Catherine's Oasis Contact Form",
  ...
  results.forEach((r, i) => {
    if (r.status === 'rejected') console.error(`Contact email #${i} failed:`, r.reason?.message);
  });
